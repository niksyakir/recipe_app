...
class RecipeFormScreen extends StatefulWidget {
  final Recipe? existingRecipe; // If null, we are creating a new recipe. If not null, we are editing.
...
class _RecipeFormScreenState extends State<RecipeFormScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.existingRecipe == null ? 'New Recipe' : 'Edit Recipe'),
      ),
      body: ListView(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value: _selectedType ?? (provider.recipeTypes.isNotEmpty ? provider.recipeTypes.first : null),
            decoration: const InputDecoration(labelText: 'Recipe Category', border: OutlineInputBorder()),
          ...
          ..._buildDynamicFields(_ingredientControllers, 'e.g., 2 cups of flour'),
          const SizedBox(height: 24),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Instructions Steps', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              TextButton.icon(
          ...
          ..._buildDynamicFields(_stepControllers, 'e.g., Bake for 20 minutes', isNumbered: true),
  ...
  List<Widget> _buildDynamicFields(List<TextEditingController> controllers, String hint, {bool isNumbered = false}) {
    return List.generate(controllers.length, (i) {
      return Padding(
        padding: const EdgeInsets.symmetric(vertical: 4),
        child: Row(
          children: [
            if (isNumbered) Padding(
